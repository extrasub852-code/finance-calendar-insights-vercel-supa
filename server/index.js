...
if (!process.env.VITEST && !process.env.VERCEL) {
    app.listen(PORT, "0.0.0.0", () => {
        const hasSb = Boolean(process.env.SUPABASE_URL?.trim() || process.env.VITE_SUPABASE_URL?.trim()) &&
            Boolean(process.env.SUPABASE_ANON_KEY?.trim() ||
                process.env.VITE_SUPABASE_ANON_KEY?.trim());
        console.log(`[fci] listening on 0.0.0.0:${PORT} node=${process.version} supabase=${hasSb}`);
    });
}
export { app };
