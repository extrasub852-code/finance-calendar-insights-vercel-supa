...
function createPrisma(): PrismaClient {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error(
      "Missing DATABASE_URL. For Turso, set TURSO_DATABASE_URL and TURSO_AUTH_TOKEN (DATABASE_URL is optional). For SQLite, set DATABASE_URL (e.g. file:./prisma/dev.db).",
    );
  }
