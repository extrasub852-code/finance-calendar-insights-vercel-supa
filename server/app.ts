...
import type { UpdateEventPatch } from "./repository.js";
...
function infraAuthHttpError(e: unknown): { status: number; error: string } | null {
...
function reqTokenOrThrow(req: express.Request): string {
  const token = req.accessToken ?? req.session?.auth?.access_token;
  if (!token) throw new Error("missing_access_token");
  return token;
}

async function ensureDefaultCategories(accessToken: string, userId: string) {
...
async function getMergedBudgets(accessToken: string, userId: string, yearMonth: string) {
...
async function getCategoriesForUser(accessToken: string, userId: string) {
...
app.get("/api/health/db", async (_req, res) => {
  try {
    ...
    if (ok) res.json({ ok: true, database: true });
    else res.status(503).json({ ok: false, database: false });
  } catch (e) {
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const user = await repo.loadAppUser(
      data.session.access_token,
      data.user.id,
      data.user.email ?? norm,
    );
    if (!user) {
...
app.post("/api/categories", requireAuth, async (req, res) => {
  try {
    ...
    const cat = await repo.createUserCategoryWithBudget(
      accessToken,
      user.id,
      slug,
